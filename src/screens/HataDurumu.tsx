...
export function HataDurumu(props: HataDurumuProps) {
  const { onRetry, onGoHome, onNavigate } = props;
    ...
    <>
      <header className="w-full top-0 sticky bg-[#131318] flex justify-between items-center px-6 py-4 z-50">
        ...
        <div className="flex items-center gap-4">
          <button
            ...
            className="hover:bg-[#1b1b20] transition-colors p-2 rounded-full scale-95 duration-200 text-[#e4e1e9] cursor-pointer"
          >
      ...
      <main className="flex-grow flex flex-col items-center justify-center p-6 text-center w-full max-w-lg mx-auto relative">
        ...
        <div className="w-full flex flex-col gap-4 sm:flex-row sm:justify-center items-center">
          <button
            onClick={onRetry}
            className="w-full sm:w-auto px-10 py-4 bg-gradient-to-br from-primary to-primary-container text-on-primary font-bold text-lg rounded-xl hover:shadow-[0_0_20px_rgba(75,226,119,0.3)] transition-all active:scale-95 duration-150 cursor-pointer"
          >
            Yeniden Dene
          </button>
          <button
            onClick={onGoHome}
            className="w-full sm:w-auto px-8 py-4 bg-surface-container-low text-on-surface font-semibold text-base rounded-xl border border-outline-variant/15 hover:bg-surface-container-high transition-colors active:scale-95 duration-150 cursor-pointer"
          >
            Ana Sayfa
          </button>
        </div>
